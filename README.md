...
These are features that only sort of work and may not continue to subsequent versions.
  ...
  **Expo+Log Hybrid Mode**- If the experimental **Expo+Log** hybrid is enabled, the log-like curve resulting from preroll and log controls can be blended with the Main roll using the 'Blend_log_into_Rolloff' slider.  Note that preroll affects both sides of the blend and the result can be kind of ugly.  Like I said, experimental.
